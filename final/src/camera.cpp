...
void camera::getAngel(k4a::device &device)
{
  ...
  if (imu_sample != NULL)
  {
    ...
    float angel[1] = {-atan(imu_sample->acc_sample.xyz.x / pow(pow(imu_sample->acc_sample.xyz.z, 2) + pow(imu_sample->acc_sample.xyz.y, 2), 0.5)) * 180 / PI};

    // libtty_write(fd, angel, buff);
  }
}

void camera::picture_update(k4a::device &device, k4a::capture &capture)
{
  device.get_capture(&capture, std::chrono::milliseconds(10));
}
